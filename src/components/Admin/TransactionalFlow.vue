            ...
            <div @click="$emit('select-order', t)" 
              ...
              :class="selectedOrder?.id === t.id ? 'border-[var(--primary-gold)]/40 shadow-[0_0_30px_rgba(184,134,11,0.1)]' : 'border-transparent hover:border-[#d4af3722] hover:shadow-2xl'">
               ...
               <div class="flex flex-col lg:flex-row items-center lg:items-center w-full gap-8 lg:gap-0">
                  ...
                  <div class="flex-grow grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 items-center gap-x-6 gap-y-8 lg:gap-6 w-full text-center sm:text-left lg:ml-12 pb-4 lg:pb-0">
                     ...
                     <div class="col-span-2 md:col-span-2">
                        <p class="text-[8px] font-bold text-stone-400 uppercase mb-2 leading-none tracking-[0.3em]">PRODUCTS ORDERED</p>
                        <div class="flex flex-wrap gap-1.5 justify-center sm:justify-start">
                          <span v-for="(item, idx) in t.items" :key="idx" 
                            class="inline-flex items-center gap-1.5 px-3 py-1 bg-[var(--primary-gold)]/5 border border-[var(--primary-gold)]/10 text-[9px] font-bold text-stone-700 dark:text-stone-300 uppercase tracking-wide rounded-sm">
                            <font-awesome-icon icon="fa-solid fa-shirt" class="text-[7px] text-[var(--primary-gold)]/50" />
                            {{ item.name }}
                            <span class="text-[7px] text-stone-400">×{{ item.quantity }}</span>
                          </span>
                        </div>
                     </div>
                     <div>
                        <p class="text-[8px] font-bold text-stone-400 uppercase mb-2 leading-none tracking-[0.3em]">GROSS TOTAL</p>
                        <p class="text-lg font-light dark:text-white font-playfair italic tracking-tighter">Rs. {{ t.amount.toLocaleString() }}</p>
                        <p class="text-[7px] font-bold text-stone-400 uppercase mt-1">incl. Rs. {{ (t.deliveryCharge || 0).toLocaleString() }} delivery</p>
                     </div>
                     <div class="col-span-2 md:col-span-1 flex justify-center sm:justify-start lg:justify-end gap-4 mt-4 lg:mt-0">
               ...
               <div class="mt-8 pt-8 border-t border-[#d4af3711] flex flex-wrap items-center gap-8 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                 @click.stop>
                  ...
                  <button @click="$emit('select-order', t)" class="ml-auto text-[9px] font-bold text-[var(--primary-gold)] uppercase tracking-[0.3em] hover:underline flex items-center gap-2">
...
defineEmits(['update-status', 'update-tracking', 'refresh', 'select-order', 'close-order'])
</script>
